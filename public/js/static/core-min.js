...
let innerWidth = 0,
    ...
    open = !1,
    PERF = 0;
const PERFS = { PERF_BAD: 0, PERF_LOW: 1, PERF_GOOD: 2, PERF_HIGH: 3 },
    getPerfs = () => {
        let e = [],
            t = PERFS.PERF_BAD;
        ...
        (t =
            deviceSettings.isTablet || deviceSettings.isIE
                ? PERFS.PERF_BAD
                : n < 5
                ? PERFS.PERF_HIGH
                : n < 14
            ...
            navigator &&
                ...
                (navigator.connection.effectiveType, (t = PERFS.PERF_BAD)),
            (PERF = t);
        const o =
            t == PERFS.PERF_HIGH
                ? "PERF_HIGH"
...
const hasWebP = () => {
    ...
    HAS_SMOOTH =
        ...
        !deviceSettings.isFirefox &&
        PERF === PERFS.PERF_HIGH,
...
const header = getId("header"),
    ...
    speed =
        parallax && parallax.hasAttribute("data-speed")
            ? parallax.getAttribute("data-speed") || 0
            : null,
    scale =
        parallax && parallax.hasAttribute("data-scale")
            ? parallax.getAttribute("data-scale")
            : 0.05,
    TIMELINES = {
        globalIn: new TimelineMax({ paused: !0 }),
        count: new TimelineMax({ paused: !0 }),
    },
    SpaceX = () => {
        decodeFeatureImage(),
            decodeImages(),
    ...
    addEventListeners = () => {
        ...
        const a = queryAll("[data-video]"),
            n = (e) => {
                ...
                openModal({ url: t, title: a }), e.preventDefault();
            };
        a &&
            a.forEach((e) => {
    ...
    animateIn = () => {
        if (
            (TIMELINES.globalIn.add(
                () => getId("wrapper").classList.add("visible"),
                0.1
            ),
            TIMELINES.globalIn
                .fromTo(
                ...
                .fromTo(
                    ...
                    3,
                    { y: -40 },
                    { y: -30, repeat: -1, ease: Expo.easeInOut },
                    1
                )
                .restart(),
            stats &&
                supportsIO &&
                (TIMELINES.count.fromTo(
                    "#stats .number",
                    0.4,
                    { autoAlpha: 0 },
                    { autoAlpha: 1 },
    ...
    globalRAF = () => {
        if (
            ((SMOOTH_SCROLLING += (cy - SMOOTH_SCROLLING) * EASE),
            HAS_SMOOTH && scroller
                ? ((scroller.style.transform = `translate3d(0,-${toFixed(
                      SMOOTH_SCROLLING,
                      IS_SCROLLING ? 3 : 0
                  )}px,0)`),
                  (scroller.style["will-change"] = IS_SCROLLING
                      ? "transform"
                      : ""),
                  (document.body.style["pointer-events"] = IS_SCROLLING
                      ? "none"
                      : ""))
                : (SMOOTH_SCROLLING = cy),
            PERF >= PERFS.PERF_HIGH &&
                parallax &&
                SMOOTH_SCROLLING <= innerHeight)
        ) {
            const e = Math.round(Math.abs(SMOOTH_SCROLLING)),
                t = 0.25 * SMOOTH_SCROLLING,
                a = SMOOTH_SCROLLING * speed,
                n = 1 + Math.max(0, Math.min(e / innerHeight, 1)) * scale;
            if (
                ((parallax.style.transform = `translate3d(0,${t}px,0) scale3d(${n}, ${n}, ${n}) rotate3d(0, 0, 0.75, ${a}deg)`),
                opacity)
            ) {
...
const limit = header.hasAttribute("data-limit")
        ? header.getAttribute("data-limit")
        : 0.45,
    headerStates = () => {
        if (!hasBackground) {
            const e = cy > ly ? "down" : "up";
            cy >= innerHeight * limit && "down" === e
                ? header.classList.add("is-hidden")
                : header.classList.remove("is-hidden"),
                cy >= innerHeight && 0 !== innerHeight && "up" === e
    ...
    inViewport = (e, t, a) => {
        TweenMax.fromTo(
            ...
            0.85,
            { autoAlpha: 0 },
            { autoAlpha: 1, ease: Power3.easeOut, delay: a }
        ),
            TweenMax.fromTo(
                e,
                1.1,
                { y: t },
                { y: 0, ease: Power4.easeOut, delay: a, force3D: !0 }
            );
    },
    addIO = () => {
        ...
        const e = deviceSettings.isMobile ? 30 : 80,
            ...
            a = [],
            n = queryAll(".animate");
        n.forEach((e) => a.push({ el: e, y: 0, ratio: 0, animated: !1 })),
            n.forEach((e) => TweenMax.set(e, { autoAlpha: 0 }));
            ...
            new IntersectionObserver((e) => {
                e.forEach((e) => {
                    e.intersectionRatio > 0
                        ? TIMELINES.count.play()
                        : TIMELINES.count.pause(0);
                });
            }, t).observe(i);
    },
    menu = () => {
        const e = new TimelineMax({ paused: !0 });
        open
            ? (getId("hamburger").setAttribute("aria-expanded", "false"),
              (open = !1),
              e
                  .to(
                      "#bar1, #bar3",
                      0.5,
                      { rotation: 0, y: 0, ease: Expo.easeOut },
                      0
                  )
                  .to(
                      "#bar2",
                      0.5,
                      { width: "100%", x: 0, ease: Expo.easeOut },
                      0
                  )
                  .to(
                      "#menu-close",
                      0.5,
                      { autoAlpha: 0, onComplete: () => (closed = !1) },
                      0
                  )
                  .to("#menu", 1, { width: 0, ease: Expo.easeOut }, 0)
                  .to("#menu-navigation", 0.5, { autoAlpha: 0 }, 0)
                  .to(
                      ".primary, .secondary",
                      0.1,
                      { autoAlpha: 0, overwrite: !0, ease: Linear.easeNone },
                      0
                  ))
            : (getId("hamburger").setAttribute("aria-expanded", "true"),
              (open = !0),
              e
                  .to(
                      "#bar1",
                      0.5,
                      { rotation: 45, y: 4, ease: Expo.easeOut },
                      0
                  )
                  .to(
                      "#bar2",
                      0.5,
                      { width: "0%", x: 8, ease: Expo.easeOut },
                      0
                  )
                  .to(
                      "#bar3",
                  ...
                  .to("#menu-navigation", 0.5, { autoAlpha: 1 }, 0)
                  .staggerFromTo(
                      innerWidth <= 960 ? ".primary, .secondary" : ".secondary",
                      1,
                      { y: 15, autoAlpha: 0 },
...
const modal = getId("modal"),
    ...
    openModal = ({ url: e = "sOpMrVnjYeY", title: t = "" }) => {
        ...
        const a = new TimelineMax({ paused: !0 }).set(
            modal,
        ...
        a.to(
            queryAll(".section"),
            1.25,
            { y: -0.25 * innerHeight, ease: Quint.easeInOut },
            0
        ),
            a
                .staggerFromTo(
                    [modal, modalchild],
                    1.25,
                    { cycle: { y: [innerHeight, -0.5 * innerHeight] } },
                    { y: 0, ease: Quint.easeInOut },
                    0,
                    0
                )
                .fromTo(
                    ...
                    { rotation: 0, autoAlpha: 1, ease: Expo.easeOut },
                    0
                )
                .add(() => {
                    query(
    ...
    closeModal = () => {
        ...
        a.fromTo(
            ...
            1,
            { y: 0.5 * innerHeight },
            { y: 0, ease: Quint.easeInOut, clearProps: "transform" },
            0
        ),
            a
                .staggerTo(
                ...
                .set(e, { autoAlpha: 0, display: "none" })
                .restart();
    },
    set = (e, t) => TweenMax.set(e, { height: t }),
    clear = (e) => TweenMax.set(e, { clearProps: "all" });
let showed = !1,
    cachedHeight = null,
    cachedWidth = null,
    minHeight = 600;
const doggyland = ({ innerHeight: e, innerWidth: t }) => {
    ...
    onResize = () => {
        if (
            ...
            showed
                ? innerHeight > cachedHeight &&
                  innerWidth === cachedWidth &&
                  (innerHeight = cachedHeight)
                : ((showed = !0),
                  doggyland({
            ...
            deviceSettings.isMobile ||
                ...
                set("#slider", innerHeight),
            innerWidth <= 600 && clear(".collapse .resize"),
            HAS_SMOOTH &&
                scroller &&
                ((bounding = scroller.getBoundingClientRect().height),
            ...
            queryAll(".slider").forEach((e) => {
                ...
                if (
                    !e.hasAttribute("data-vehicle") &&
                    t &&
                    t.classList.contains("not-mobile")
                )
                    return;
